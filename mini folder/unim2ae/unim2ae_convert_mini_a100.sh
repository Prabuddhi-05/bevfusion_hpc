...
srun \
  ...
  bash -lc '
    ...
    # ---------------- Convert UniM2AE checkpoint to BEVFusion format ----------------
    ...
    SRC_CKPT="work_dirs/unim2ae_mmim/epoch_200.pth"
    TGT_DIR="../Finetune/bevfusion/pretrained"
    mkdir -p "$TGT_DIR"
    TGT_CKPT="$TGT_DIR/unim2ae-pre.pth"

    if [ ! -f "$SRC_CKPT" ]; then
      echo "[ERROR] Source checkpoint not found at: $SRC_CKPT"
      echo "[IN-CTR] Listing work_dirs/unim2ae_mmim for debugging:"
      ls -la work_dirs/unim2ae_mmim || true
      exit 1
    fi

    echo "[IN-CTR] Running convert.py ..."
    echo "[IN-CTR] Source: $SRC_CKPT"
    echo "[IN-CTR] Target: $TGT_CKPT"

    python tools/convert.py --source "$SRC_CKPT" --target "$TGT_CKPT"

    echo "[IN-CTR] Conversion finished. Output: $TGT_CKPT"
  '

#!/bin/bash
#SBATCH --job-name=unim2ae_mmim_eval
#SBATCH --output=logs/unim2ae_mmim_eval_%j.out
...
echo "[INFO] Starting UniM2AE MMIM camera+LiDAR EVAL on nuScenes mini (container)..."
...
srun \
  ...
  bash -lc '
    ...
    # Torchpack for BEVFusion training/eval
    ...
    # ---------------- EVALUATION ----------------
    echo "[IN-CTR] Starting MMIM camera+LiDAR UniM2AE BEVFusion EVAL on nuScenes mini ..."
    torchpack dist-run -np 4 python tools/test.py \
      ...
      runs/run-4c5379ec-8bc22e8e/epoch_6.pth \
      --eval bbox

    echo "[IN-CTR] Evaluation finished."
  '

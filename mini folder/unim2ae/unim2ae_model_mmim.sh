...
srun \
  ...
  bash -lc '
    ...
    # ---------------- Convert LiDAR-only checkpoint to stage1 ----------------
    echo "[IN-CTR] Converting LiDAR-only UniM2AE-L checkpoint to stage1 fused weights ..."
    python tools/convert.py \
      --source runs/run-4c5379ec-7d3d5bbc/epoch_20.pth \
      --fuser pretrained/unim2ae-pre.pth \
      --target pretrained/unim2ae-stage1.pth \
      --stage2

    # ---------------- MMIM camera+LiDAR training ----------------
    echo "[IN-CTR] Starting MMIM camera+LiDAR UniM2AE BEVFusion training on nuScenes mini ..."
    torchpack dist-run -np 4 python tools/train.py \
      configs/nuscenes/det/transfusion/secfpn/camera+lidar/swint_v0p075/unim2ae_MMIM.yaml \
      --load_from pretrained/unim2ae-stage1.pth

    echo "[IN-CTR] MMIM camera+LiDAR training finished."
  '

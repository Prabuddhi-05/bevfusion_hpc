...
srun \
  ...
  bash -lc '
    ...
    # ---------------- Visualize pretraining with test.py ----------------
    ...
    CKPT_PATH="work_dirs/unim2ae_mmim/epoch_200.pth"
    if [ ! -f "$CKPT_PATH" ]; then
      echo "[ERROR] Pretrain checkpoint not found at: $CKPT_PATH"
      echo "[IN-CTR] Listing work_dirs/unim2ae_mmim for debugging:"
      ls -la work_dirs/unim2ae_mmim || true
      exit 1
    fi

    echo "[IN-CTR] Running tools/test.py with --show-pretrain ..."
    echo "[IN-CTR] Checkpoint: $CKPT_PATH"

    mkdir -p viz

    python tools/test.py configs/unim2ae_mmim.py \
      --checkpoint "$CKPT_PATH" \
      --show-pretrain --show-dir viz

    echo "[IN-CTR] test.py finished. Visualizations saved under: Pretrain/viz"
  '

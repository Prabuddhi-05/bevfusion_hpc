#!/bin/bash
#SBATCH --job-name=unim2ae_lidar_mini
#SBATCH --output=logs/unim2ae_lidar_mini_%j.out
...
echo "[INFO] Starting UniM2AE LiDAR-only training on nuScenes mini (container)..."
...
srun \
  ...
  bash -lc '
    ...
    # *** NEW: install mpi4py so torchpack.distributed can import it ***
    ...
    echo "[IN-CTR] Starting LiDAR-only UniM2AE-L training on nuScenes mini ..."
    torchpack dist-run -np 4 python tools/train.py \
      configs/nuscenes/det/transfusion/secfpn/lidar/sstv2.yaml \
      --load_from pretrained/unim2ae-lidar-only-pre.pth

    echo "[IN-CTR] LiDAR-only training finished."
  '

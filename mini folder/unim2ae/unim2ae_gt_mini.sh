...
echo "[INFO] Starting UniM2AE job (container, create GT DB only)..."
...
srun \
  ...
  bash -lc '
    ...
    # ---------------- Install UniM2AE Pretrain code (unchanged) ----------------
    cd Pretrain
    python setup.py develop --no-deps

    # ---------------- Install Finetune/bevfusion code (for create_gt) ---------
    cd ../Finetune/bevfusion
    python setup.py develop --no-deps

    # ---------------- Create nuScenes GT database (Finetune/bevfusion) --------
    ...
    echo "[IN-CTR] Running tools/create_gt.py for nuScenes v1.0-mini ..."
    python tools/create_gt.py nuscenes \
      --root-path ./data/nuscenes \
      --version v1.0-mini \
      --out-dir ./data/nuscenes \
      --extra-tag nuscenes

    echo "[IN-CTR] Ground truth database creation finished."
  '

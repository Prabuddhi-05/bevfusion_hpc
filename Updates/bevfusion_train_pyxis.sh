...
#SBATCH --nodelist=hpc-novel-gpu04
...
srun --ntasks=1 --gpus=4 --gpu-bind=closest --mpi=none \
  ...
  bash -lc '
    ...
    # ---------------- monkey patch TensorBoard ----------------
    echo "[IN-CTR] Monkey patching TensorBoard at runtime..."
    cat <<'"'"'PATCH'"'"' > /workspace/patch_tensorboard.py
import sys
import importlib
import tensorboard
try:
...
PATCH
    ...
    python -c '\''import patch_tensorboard; print("[IN-CTR] TensorBoard runtime patch applied")'\''
    ...
    HOST=$(hostname)
    echo "[IN-CTR] Using host $HOST with 4 slots"

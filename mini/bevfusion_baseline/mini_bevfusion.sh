...
#SBATCH --output=logs/mini_bevfusion_%j.out
#SBATCH --time=6-00:00:00
#SBATCH --mem=128G
#SBATCH --ntasks=1                        # ✅ 1 task only
#SBATCH --cpus-per-task=32
#SBATCH --partition=gpu
#SBATCH --nodes=1
#SBATCH --gpus=nvidia_a100:4              # ✅ request 4 GPUs
...
srun --ntasks=1 --gpus=4 --gpu-bind=closest --mpi=none \
  ...
  bash -lc '
    ...
    torchpack dist-run -np 4 -H ${HOST}:4 python tools/train_no_tensorboard.py \
      configs/nuscenes/det/transfusion/secfpn/camera+lidar/swint_v0p075/convfuser_mini.yaml \
      --load_from pretrained/mini/mini_fusion_init.pth
  '

...
#SBATCH --output=logs/mini_camera_only_eval_%j.out
...
enroot start \
  ...
  "$CONT_NAME" bash -lc '
    ...
    torchpack dist-run -np 2 python tools/test.py \
      configs/nuscenes/det/centerhead/lssfpn/camera/256x704/swint/default_mini.yaml \
      runs/run-fa8ecad4/epoch_20.pth \
      --eval bbox > runs/run-fa8ecad4/eval_epoch_20_newrun.txt 2>&1
  '
